...
CsdnUbbEditor.prototype.smiliesLayer = function()
{
	var item = "<td title=\"{1}\" onclick=\"CsdnUbbEditor.instance('" + this.hashCode + 
		"').smilies('{0}')\"><img src='/UBB/smilies/xiha/{0}.jpg' /></td>";
	var div = document.createElement("DIV"), a = [], me = this;
	div.className = "CsdnUbbEditorSmiliesLayer";
	div.style.display = "none";
	div.id = "CsdnUbbEditorSmiliesLayer_" + this.hashCode;
	div.onclick = function(e) { (window.event || e).cancelBubble = true; }
	a.push("<table border='0' cellpadding='0' cellspacing='2'>");
	a.push("<tr>");
	a.push(item.format("a01", "\u62cd\u638c"));
	a.push(item.format("a02", "\u8ba4\u771f"));
	a.push(item.format("a03", "\u5bd2"));
	a.push(item.format("a04", "\u4e0d\u5173\u6211\u4e8b"));
	a.push(item.format("a05", "\u88c5\u9177"));
	a.push(item.format("a06", "\u65e0\u77e5"));
	a.push(item.format("a07", "\u5927\u54ed"));
	a.push(item.format("a08", "\u79f0\u8d5e"));
	a.push("</tr>");
	a.push("<tr>");
	a.push(item.format("a09", "\u53f9\u6c14"));
	a.push(item.format("a010", "\u50bb\u7b11"));
	a.push(item.format("a011", "\u6655"));
	a.push(item.format("a012", "\u95ed\u5634"));
	a.push(item.format("a013", "\u7591\u95ee"));
	a.push(item.format("a014", "\u5978\u7b11"));
	a.push(item.format("a015", "\u5e86\u795d"));
	a.push(item.format("a016", "\u8def\u8fc7"));
	a.push("</tr>");
	a.push("</table>");
	a=a.join("");
	div.innerHTML = a;
...
CsdnUbbEditor.directorHTML = 
{
	...
	smilies : t.format("{0}", "smilies", "\u63d2\u5165\u8868\u60c5"),
...
CsdnUbbEditor.prototype.render = function(ubblist)
{
	if ("string" != typeof(ubblist) || ubblist.length == 0)
	{
		ubblist = "fontsize|space|bold|italic|underline|strikethrough|color|code|space" +
			"|url|email|image|smilies|swf|movie|space|left|center|right|space|help|increase|decrease";
	}
...
CsdnUbbEditor.prototype.smilieslayer = function(button)
{
	var xy = CsdnUbbEditor.realOffset(button);
	var layer = CsdnUbbEditor.check("CsdnUbbEditorSmiliesLayer_" + this.hashCode);
	layer.style.top = (xy.y + button.offsetHeight) + "px";
	layer.style.left = xy.x + "px";
	this.showLayer(layer);
};

CsdnUbbEditor.prototype.smilies = function(em)
{
	this.hideLayer("CsdnUbbEditorFontsizeLayer_" + this.hashCode);
	var rule = CsdnUbbEditor.director["smilies"];
	this.wraptag(rule.open.format(em), rule.close);
};
