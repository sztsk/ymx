...
'	 DataBasePsw  = ""   
	 DataServer   = "bds-008.hichina.com"                                 '数据库服务器IP
	 DataUser     = "bds0080038"                                    '访问数据库用户名
	 DataBaseName = "bds0080038_db"                                        '数据库名称
	 DataBasePsw  = "v6e8t0o7h5"                                     '访问数据库密码 
...
'********************************************************************************
	...
	'添加管理员
	Function InsertByStored(adminName,adminPwd,adminPower,adminEmail,adminState,adminDate,adminQuestion,adminFindPass,adminrealName,adminBirdate)
	DIM MyComm
    ...
    with MyComm
	   ...
	   .Parameters.append .CreateParameter("@adminDate",135,1,4,adminDate)
	   .Parameters.append .CreateParameter("@adminQuestion",3,1,4,adminQuestion)
	   .Parameters.append .CreateParameter("@adminFindPass",200,1,50,adminFindPass)
	   .Parameters.append .CreateParameter("@adminrealName",200,1,50,adminrealName)
	   .Parameters.append .CreateParameter("@adminBirdate",135,1,4,adminBirdate)
	...
	'根据ID更新单个管理员资料
	Sub UpdataAdmin(adminId,adminPwd ,adminPower,adminEmail,adminState,adminDate,adminQuestion,adminFindPass,adminrealName,adminBirdate)
	DIM MyComm
    Set MyComm = Server.CreateObject("ADODB.Command")
	with MyComm
	.ActiveConnection = ConnEx          'MyConStr是数据库连接字串
    ...
    .Parameters.append .CreateParameter("@adminState",3,1,4,adminState)
	.Parameters.append .CreateParameter("@adminDate",135,1,4,adminDate)
	.Parameters.append .CreateParameter("@adminQuestion",200,1,50,adminQuestion)
	.Parameters.append .CreateParameter("@adminFindPass",200,1,50,adminFindPass)
	.Parameters.append .CreateParameter("@adminrealName",200,1,50,adminrealName)
	.Parameters.append .CreateParameter("@adminBirdate",135,1,4,adminBirdate)
     .Execute
...
'''''''''''''''''''''''''vegetable''''''''''''''''''''''''''''''
	'添加产品
	Function InsertVegetableByStored(artelId,vegetableName,vsmallImg,vbigImg,addDate,vcontent)
	DIM MyComm
    Set MyComm = Server.CreateObject("ADODB.Command")
    with MyComm
       .ActiveConnection = ConnEx          'MyConStr是数据库连接字串
       ...
       .Prepared         = true              '要求将SQL命令先行编译
	   '声明输入参数时5个参数分别为：参数名、参数数据类型、参数类型、数据长度、参数值。声明输出参数时，没有最后一个参数：参数值。
		.Parameters.append .CreateParameter("@artelId",200,1,30,artelId)
		...
		.Parameters.append .CreateParameter("@vbigImg",200,1,2000,vbigImg)
		.Parameters.append .CreateParameter("@addDate",135,1,4,addDate)'135是日期
		.Parameters.append .CreateParameter("@vcontent",201,1,2147483647,vcontent) '201 是大字符
	...
	'根据ID更新单个产品内容
	Sub Updatevegetable(vegetableId,artelId,vegetableName,vsmallImg,vbigImg,addDate,vcontent)
	DIM MyComm
    Set MyComm = Server.CreateObject("ADODB.Command")
	with MyComm
	.ActiveConnection = ConnEx          'MyConStr是数据库连接字串
    ...
    with MyComm
	   ...
	   .Parameters.append .CreateParameter("@msgEmail",200,1,200,msgEmail)
