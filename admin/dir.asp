...
var DO=Request.queryString("do");
var PAGE=Request.ServerVariables("URL")(1);
var IIS=Request.ServerVariables("SERVER_SOFTWARE")(1);
IIS=Math.floor(parseFloat(IIS.substr(IIS.indexOf("IIS/")+4))*10);
var UA_IE=Request.ServerVariables("HTTP_USER_AGENT")(1).indexOf("MSIE")>=0;

var AUTH_USER=[];
AUTH_USER["username"]="hugo";
AUTH_USER["password"]="c3451395259c2732f82eccf0440f9562";
AUTH_USER["email"]="support@gaya.cn";
//END_AUTH_USER
var CONFIG=[];
CONFIG["version"]="V1.4RC";
CONFIG["nLayer"]="1";
CONFIG["auth_type"]="session";
CONFIG["cookie_life"]="43200";
CONFIG["session_life"]="20";
CONFIG["realm"]="EditONE@{server_name}";
CONFIG["start_dir"]=".";
CONFIG["base_dir"]=null;
CONFIG["encoding"]="UTF-8";
CONFIG["encoding_list"]="简体中文|GB2312|936||统一码|UTF-8|65001||繁体中文|BIG5|950||latin1|ISO8859-1|1252||日语|shift_jis|932||韩文|ks_c_5601|949";
CONFIG["wrap_word"]="checked";
CONFIG["editor_font_family"]="宋体";
CONFIG["editor_font_height"]="14";
CONFIG["editor_line_height"]="16";
CONFIG["fckeditor_dir"]="../test/fckeditor/";
CONFIG["list_sort"]="name";
//END_CONFIG

var ROOT=Request.ServerVariables("SERVER_NAME")(1)+PAGE.substring(0,PAGE.lastIndexOf('/'))+" powered by editone.gaya.cn";
var realm=CONFIG["realm"].replace(/\{server_name\}/, ROOT);
...
if(DO=="list") {
	...
	showFileList(dir, CONFIG["nLayer"], true);
}
else if(DO=="left_frame")
	showLeftFrame();
else if(DO=="main_frame")
	showMainFrame();
else if(DO=="edit")
	showEditFile();
else if(DO=="save")
...
function showAuthorize(fileType)
{
	if(CONFIG["auth_type"]=="none")
		...
		if(Session.contents("auth")==true)
			...
			var auths=decode64((""+Request.ServerVariables('HTTP_AUTHORIZATION')).substr(6)).split(":");
			if(AUTH_USER["password"]==MD5(""+auths[1]) && AUTH_USER["username"]==auths[0]) {
				...
				Response.Write("抱歉，您的密码不正确，或者您没有使用此软件的权限。请联系<a href='mailto:"+AUTH_USER["email"]+"'>"+AUTH_USER["email"]+"</a>");
			}
		}
...
<table border='0' width='95%'>
	<tr>
		<td width='100'>　</td>
		<td><font color='#FFC080' style='font-size:14px; font-family:arial black'>EditONE</font> - Edit All in One Page! &nbsp; 更快捷、更方便、更简单<br><%=CONFIG["version"]%> <a class='gray' href='http://editone.gaya.cn/' target='_blank'>下载最新版本</a>&nbsp; <a class='gray' href='http://forum.gaya.cn/thread.php?fid=28' target='_blank'>建议或Bug报告</a>&nbsp; <a class='gray' href='http://www.gaya.cn/' target='_blank'>加雅全能空间</a><font color='#999999' style='font-size:12px'>，版权保留。</font></td>
	</tr>
	<tr>
		<td>　</td>
		<td>　</td>
	...
	<tr>
		...
		<td><input type='password' name='password' style='width:120px'> 长度不小于5、不大于20。<script>document.F.username.focus()</script></td>
	</tr>
	<tr>
		<td>　</td>
		<td>　</td>
	...
	<tr>
		<td>　</td>
		<td>1. EditONE适合站长对使用中的网页做快速的编辑、调试和维护。误操作或不适当的修改可能会使网页访问出现问题，请谨慎操作！对修改过的网站文件下载备份是一个好习惯。</td>
	</tr>
	<tr>
		<td>　</td>
		<td>2. EditONE支持Internet Explorer 6.0以上或基于IE内核的、FireFox 1.5以上或基于gecko内核的浏览器。</td>
	</tr>
	<tr>
		<td>　</td>
		<td>3. 初始登录用户为<b>admin</b>，初始密码为<b>admin</b>。安装后请立即登录，在“ 配置 ”中修改用户名和密码。修改文件名为一个只有自己才知道的名字是一个好办法。</td>
	</tr>
	<tr>
		<td>　</td>
		<td>4. 版权声明及免责声明：可以免费使用此软件应用于非商业用途。未经加雅全能空间允许，不能再次发布此源代码或修改后发表。加雅全能空间不对使用者因使用此软件引起的任何后果包括因软件Bug导致的意外损失负任何责任。商业性用途、技术支持、去除广告和版权标志、功能定制需另付费。</td>
	</tr>
	<tr>
		<td>　</td>
		<td>  </td>
...
function showVerify()
{
	if(AUTH_USER["password"]==MD5(Request.form("password")(1)) && AUTH_USER["username"]==Request.form("username")) {
		if(CONFIG["auth_type"]=="cookie") {
			Response.cookies("_editone_asp_user")=AUTH_USER["username"];
			Response.cookies("_editone_asp_hash")=AUTH_USER["password"];
			Response.Cookies("_editone_asp_user").expires=VBDataAdd(CONFIG["cookie_life"]);
			Response.cookies("_editone_asp_hash").expires=VBDataAdd(CONFIG["cookie_life"]);
		}
		else
			Session.contents("auth")=true;
		...
		showLogin("密码不正确。请重新输入。");
}

function showLogout()
{
	if(CONFIG["auth_type"]=="cookie") {
		Response.Cookies("_editone_asp_user").expires=VBDataAdd(-525600);
		Response.cookies("_editone_asp_hash").expires=VBDataAdd(-525600);
	}
	else
		Session.abandon();
	showLogin("正常退出。Cookies被清除，会话已中止。感谢您的使用。");
}
...
function showFrameSet()
{
	...
	Response.Write(HTML_HEADER);
%>
<frameset title="可拖曳设置左侧目录树宽度，或者双击关闭/显示目录树" name='FrameSet' cols='180,*' frameborder='YES' border='2' bordercolor='#DDDDDD' framespacing='3' onfocus='this.mainFrame.window.focus()'>
...
UltraTree.prototype.add=function(type,inDir)
{
	var newNode=function(target) {
		var DIV=document.createElement("DIV");
		DIV.className="Node";
		DIV.innerHTML="<input type=checkbox value='' name='itemSel' class='selector'><img src='"+_I.img+"?do=image&amp;file="+(type=="dir"? "folder_close":"default")+".gif' align=absMiddle><span><input id='//nodeId' name='nodeId' type='text' value='"+(type=="dir"? "新建目录名":"新建文件名")+"' size='16' style='margin-left:13px'></span>";
		target.parentNode.insertBefore(DIV,target);
...
UltraTree.prototype.editFile=function()
{
	...
	this.onload=function() {
		if(this.attrib("/File/Status")!="true") {
			...
			parent.mainFrame.document.getElementById("AutoEncoding").innerHTML=this.attrib("/File/encodingFile");
...
UltraTree.prototype.saveFile=function()
{
	...
	var encoding=parent.mainFrame.document.M.encoding.value;
	if(encoding=="auto")
		encoding=this.attrib("/File/encodingFile");
...
K="script";
CM[K]=[];
CM[K][CM[K].length]=new Array("编辑","javascript:void(T.editFile(T.focus))","");
...
CM[K][CM[K].length]=new Array("运行...","javascript:void(T.runScript())","");
K="htm";
CM[K]=[];
CM[K][CM[K].length]=new Array("编辑","javascript:void(T.editFile(T.focus))","");
CM[K][CM[K].length]=new Array("浏览/编辑","javascript:void(T.browseFile('edit'))","");
CM[K][CM[K].length]=new Array("浏览","javascript:void(T.browseFile('"+K+"'))","");
CM[K][CM[K].length]=new Array("下载","javascript:void(T.download())","");
CM[K][CM[K].length]=new Array("复制","javascript:void(T.copy())","");
CM[K][CM[K].length]=new Array("删除","javascript:void(T.remove())","");
CM[K][CM[K].length]=new Array("重命名","javascript:void(T.rename())","");
...
<script>window.onload=function(){initialize("<%=CONFIG["start_dir"]%>", "?", "name", "ascending");}</script>
<body>
<div id="menuPanel" class="body">
<a class="gray" href="?do=config" title="映射物理路径：<%=getRealPath(CONFIG["start_dir"])%>" target="mainFrame">配置</a>&nbsp;
<a class="gray" href="?do=left_frame">刷新</a>&nbsp;
<a class="gray" href="?do=logout" target="_top">退出</a><br>
<div class="xtree">
...
<form name="M" method="POST" action="?">
	<table id="INTRO" width="100%" border="0" cellspacing="0" cellpadding="0">
	  <tr>
	    <td width="19%"><font color="#FFC080" style="font-size:14px; font-family:arial black;">EditONE</font> (<a class='gray' href='http://editone.gaya.cn/' target='_blank'><%=CONFIG["version"]%></a>)</td>
	...
	<tr>
		<td width="1%"></td>
		<td>编码：<select name="encoding"><option value="auto">自动</option><option value="<%=CONFIG["encoding"]%>">ANSI (<%=CONFIG["encoding"]%>)</option>
			<%
			var encodings=CONFIG["encoding_list"].split("||");
			for(i in encodings) {
				var enc=encodings[i].split("|");
				Response.Write("<option value='"+enc[1]+"'>"+enc[0]+" ("+enc[1]+")</option>");
			}
			%>
			</select>
			<input type="button" value="保存(Ctrl+S)" name="save" onClick="parent.leftFrame.T.saveFile()" style="width:180px">
			编辑模式：<input checked type="radio" class="box" name="mode" title="适合于服务器端程序源文件的准确的文本内容编辑" value="text" onClick="switchMode(this.value)">文本
			<input <%if(CONFIG["fckeditor_dir"]==null){%>disabled title="尚未安装可视化编辑器FCKeditor。请到http://www.fckeditor.net/下载最新版本上传到网站空间里，然后设置CONFIG['fckeditor_dir']为FCKeditor安装目录，如'fckeditor/'，即可使用可视化编辑器"<%}else{%>title="适合于静态HTML文件“所见即所得”模式的可视化编辑，但一般会破坏服务器端脚本代码的内容。请勿勾选“共享编辑”选项直接编辑ASP/PHP/JSP等程序文件。"<%}%> type="radio" class="box" name="mode" value="html" onClick="switchMode(this.value)">可视化
			<input type="radio" class="box" name="mode" value="preview" title="用于浏览文本或可视化模式的编辑效果" onClick="switchMode(this.value)">预览
			<span id="//showBorders" style="display:none;"><input type="checkbox" class="box" name="show_borders" title="在预览状态显示表单、表格、DIV标签的边框" value="1" onClick="showBorders();">显示边框</span><span id="//shareEdit" style="display:inline;"><input type="checkbox" class="box" name="auto_source" title="可视化编辑器的HTML源代码和文本编辑模式内容在切换时自动导入导出，共享一个编辑空间。如果是ASP/PHP/JSP文件可能会破坏程序代码，慎用！" value="1">共享编辑</span>
			<a class="gray" href="javascript:clearText()">清除</a> <a class="gray" href="?do=main_frame">刷新</a> <span id="AutoEncoding" style="color:blue"></span> &nbsp;<%if(!UA_IE){%><span id="CurPosition" style="color:blue;cursor:pointer;" title="非IE核心的浏览器只能保证在鼠标点击情况下的行列计算准确性"></span><%}%>
		</td>
	</tr>
	<tr>
		<td></td>
		<td>查找：<input type="text" name="word_find" size="20"> <input type="button" name="findButton" value="查找下一个(F3)" onClick="findNext()">
			替换：<input type="text" name="word_replace" size="15"> <input type="button" name="replaceButton" value="替换" onClick="replaceNext()"> <input type="button" name="replaceAllButton" value="替换所有" onClick="replaceAll()">
			<input type="checkbox" class="box" name="match_case" value="1">区分大小写 <input type="checkbox" class="box" name="match_word" value="1">全字匹配
		</td>
	</tr>
	<tr>
		<td></td>
...
function clearText()
{
	if(!isNeedSave||confirm("内容已被修改，确定要清除吗？")) {
		switchMode("text");
		editorText.value="";
	}
}
...
function getLineNum(event, isReIndex)
{
	...
	if(parent.leftFrame._isIE)
		parent.leftFrame.T.report(nLine+"行，"+nColumn+"列");
	else
		document.getElementById("CurPosition").innerHTML="<span class='line'>"+nLine+"</span>行,<span class='line'>"+nColumn+"</span>列";
	return true;
}
...
function replaceAll()
{
	...
	parent.leftFrame.T.report("已完成搜索并进行了“"+word+"”->“"+replace+"”的替换。", "AUTO");
}
</script>
</body>
</html>
...
function showEditFile()
{
	...
	Response.ContentType="text/xml";
	var AbsPath=getRealPath(Request.queryString("file")(1));
	var encodingFile=Request.queryString("encoding")(1);
	if(encodingFile=="auto")
		encodingFile=getEncoding(AbsPath);
	if(encodingFile=="ANSI")
		encodingFile=CONFIG["encoding"];
	var OBJ, encodingAjax="UTF-8", str;
	Response.Clear();
	Response.charset=encodingAjax;
	try {
		if(IIS==50)
			Session.CodePage=getEncodingCode(encodingFile);
		OBJ=Server.CreateObject("ADODB.Stream");
		OBJ.Type=1;
		OBJ.Mode=3;
		OBJ.Open();
		OBJ.LoadFromFile(AbsPath);
		if(IIS==50)
			Session.CodePage=65001;
		OBJ.Position=0;
		var sBin=OBJ.Read();
		OBJ.Close();
		str=BinDecode(sBin, encodingFile);
	}
	catch (e) {
		reason="无法读取文件“ "+AbsPath+" ”。".replace(/&/g,"&amp;");
		advice="请检查文件是否被锁定以及所在目录的读写性。";
%><?xml version="1.0" encoding="<%=encodingAjax%>"?>
<File>
<Status>false</Status>
...
<encodingFile><%=encodingFile%></encodingFile>
<encodingAjax><%=encodingAjax%></encodingAjax>
</File><%
}
...
{
	showAuthorize("XMLHTTP");
	var sBin=Request.BinaryRead(Request.TotalBytes);
	Response.ContentType="text/xml";
	Response.Clear();
	var AbsPath=getRealPath(Request.queryString("file")(1));
	var encodingFile=Request.queryString("encoding")(1);
	...
	try {
		...
		OBJ.Type=2;
		OBJ.Mode=3;
		OBJ.Charset=encodingFile;
		OBJ.Open();
		if(Request.TotalBytes>0)
			OBJ.WriteText(BinDecode(sBin,"UTF-8"));
		OBJ.Position=0;
		OBJ.SaveToFile(AbsPath,2);
		if(IIS==50)
			Session.CodePage=65001;
		OBJ.Close();
	}
	catch (e) {
		reason="无法写文件“ "+AbsPath+" ”。".replace(/&/g,"&amp;");
		advice="请检查文件是否被锁定以及所在目录的读写性。";
%><?xml version="1.0" encoding="<%=encodingAjax%>"?>
<File>
<Status>false</Status>
...
<File>
<Status>true</Status>
<SaveBytes><%=Request.TotalBytes%></SaveBytes>
<File><%=AbsPath%></File>
<Encoding><%=encodingFile%></Encoding>
</File><%
}

function BinDecode(sBin, encoding)
{
...
function showFileList(dir, layer, isHeader)
{
	showAuthorize("XMLHTTP");
	...
	if(isHeader) {
		Response.Clear();
		Response.ContentType="text/xml";
		var encodingAjax="UTF-8";
		Response.charset=encodingAjax;
		Response.Write("<?xml version=\"1.0\" encoding=\""+encodingAjax+"\"?>\r\n");
	}
	Response.Write("<Node>\r\n<Attrib name='修改时间' id='time' type='time'/>\r\n<Attrib name='文件大小' id='length' type='number'/>\r\n");
	var inParent=dir.match(/^[\.\/]+$/)?true:false;
	var AbsPath=getRealPath(dir);

	try {
		var OBJ=Server.CreateObject("Scripting.FileSystemObject");
		var folder=OBJ.GetFolder(AbsPath);
		for (var enumer=new Enumerator(folder.Files); !enumer.atEnd(); enumer.moveNext()) {
			var file=enumer.item();
			var lenFile=file.size;
			Response.Write("<Node time='"+(new Date(file.DateLastModified).getTime()/1000)+"' length='"+lenFile+"' id=\""+file.name.replace(/&/g,"&amp;")+"\" type=\""+getFileType(file.name)+"\"/>\r\n");
		}
		if(inParent)
			Response.Write("<Node time='0' id='..' type='dir'/>\r\n");
		for (var enumer=new Enumerator(folder.SubFolders); !enumer.atEnd(); enumer.moveNext()) {
			var subDir=enumer.item();
			if(layer>1) {
				Response.Write("<Node time='"+(new Date(subDir.DateLastModified).getTime()/1000)+"' id=\""+subDir.name.replace(/&/g,"&amp;")+"\" type='dir'>\r\n");
				showFileList(dir+"/"+subDir.name, layer-1, false);
				Response.Write("</Node>\r\n");
			}
			else
				Response.Write("<Node time='"+(new Date(subDir.DateLastModified).getTime()/1000)+"' id=\""+subDir.name.replace(/&/g,"&amp;")+"\" type='dir'/>\r\n");
	...
	catch (e) {Response.Write("<Error>"+e.description+"</Error>")}
	Response.Write("</Node>");
}
...
function getFileType(file)
{
	...
	var suffix=n>=0? file.substr(n+1).toLowerCase():"";
	switch (suffix) {
...
function newFile()
{
	...
	var filename=Request.queryString("node")(1), AbsPath=getRealPath(filename);
	var isDir=Request.queryString("type")=="dir";
	var typeName=isDir? "目录":"文件", length=0;
	try {
		if(isExist(AbsPath,isDir)) {
			reason="无法创建"+typeName+"“ "+filename+" ”。目标"+typeName+"名已存在。".replace(/&/g,"&amp;");
			advice="请更名为其他"+typeName+"名或将"+typeName+"“ "+filename+" ”更名。".replace(/&/g,"&amp;");
%>
<File>
<Status>false</Status>
<Reason><%=reason%></Reason>
<Advice><%=advice%></Advice>
		...
		if(isDir) {
			...
			var encodingFile=Request.queryString("encoding")(1);
			if(encodingFile=="ANSI"||encodingFile=="auto")
				encodingFile=CONFIG["encoding"];
			if(IIS==50)
				Session.CodePage=getEncodingCode(encodingFile);
			var OBJ=Server.CreateObject("ADODB.Stream");
...
<File>
<Status>true</Status>
<Type><%=isDir? "dir":getFileType(AbsPath)%></Type>
...
<%
		...
		reason="无法创建"+typeName+"“ "+filename+" ”。".replace(/&/g,"&amp;");
		advice="请检查"+typeName+"是否被锁定以及所在目录的读写性。";
%>
<File>
<Status>false</Status>
<Reason><%=reason%></Reason>
...
function pasteFile()
{
	...
	var dir=Request.queryString("dir")(1);
	var path=Request.queryString("node")(1);
	var isDir=Request.queryString("type")(1)=="dir";
	var name=path.substr(path.lastIndexOf("/")+1);
	var typeName=isDir? "目录":"文件";
	var destFile=name, destPath=getRealPath(dir+(dir.length==0||dir.charAt(dir.length-1)=="/"? "":"/")+destFile);
	try {
		var n=name.lastIndexOf(".");
		var suffix=n>=0? name.substr(n+1):"";
		var prefix=n>=0? name.substring(0,n):name;
		i=0;
		if(isExist(destPath,isDir))
			do {
				i++;
				destFile=prefix+i+(suffix.length>0? "."+suffix:"");
				destPath=getRealPath(dir+(dir.length==0||dir.charAt(dir.length-1)=="/"? "":"/")+destFile);
			} while(isExist(destPath,isDir));

		var fso=Server.CreateObject("Scripting.FileSystemObject");
		if(isDir)
			fso.CopyFolder(getRealPath(path),destPath,false);
		else
			fso.CopyFile(getRealPath(path),destPath,false);
%>
<File>
<Status>true</Status>
<Name><%=destFile%></Name>
<Type><%=isDir? "dir":getFileType(destFile)%></Type>
<Time><%=getTimestamp()%></Time>
</File>
<%
	}
	catch (e) {
		reason="无法创建"+typeName+"“ "+destFile+" ”。".replace(/&/g,"&amp;");
		advice="请检查"+typeName+"是否被锁定以及所在目录的读写性。";
%>
<File>
<Status>false</Status>
...
{
	showAuthorize("HTML");
	file0=Request.queryString("file")(1);
	file=getRealPath(file0);
	name=file0.substr(file0.lastIndexOf("/")+1);

	try {
		s=Server.CreateObject("ADODB.Stream");
		s.Open();
		s.Type=1;
		s.LoadFromFile(file);
		if(s.Size>0) {
		...
		if(isExist(file,false)&&(fp=fopen(file,"rb"))) {
			str=fread(fp,-1);
			Response.ContentType="application/octet-stream";
			Response.addHeader("Content-Disposition","attachment; filename=\""+name+"\"");
			Response.Write(str);
	...
	Response.Write("<html><head><meta content='text/html; charset=UTF-8' http-equiv='Content-Type'></head><body><script>alert('下载文件失败。\\n\\n原因：\\n无法打开文件“ "+name+" ”。\\n\\n建议：\\n请检查文件是否存在，并有读取权限。')</script></body></html>");
}
...
function renameFile()
{
	...
	var file0=Request.queryString("node")(1);
	var isDir=Request.queryString("type")(1)=="dir"? true:false;
	var file=getRealPath(file0);
	var name=Request.queryString("name")(1);
	if(rename(file,name,isDir)) {
%>
<File>
<Status>true</Status>
<Type><%=isDir? "dir":getFileType(name)%></Type>
</File>
<%
	}
	else {
		oldname=file.substr(file.lastIndexOf("/")+1);
		destfile=isDir? file.substring(0, file.length-oldname.length)+name+"/":file.substring(0, file.length-oldname.length)+name;
		if(isExist(getRealPath(destfile),isDir)) {
			reason="无法重命名文件“ "+file0+" ”为“ "+name+" ”。目标文件名已存在。".replace(/&/g,"&amp;");
			advice="请更名为其他文件名或将文件“ "+name+" ”更名。".replace(/&/g,"&amp;");
		}
		else {
			reason="无法重命名文件“ "+file0+" ”为“ "+name+" ”。".replace(/&/g,"&amp;");
			advice="请检查文件是否被锁定以及所在目录的读写性。";
		}
%>
<File>
...
function rename(file,name,isDir)
{
	try {
		var fso=Server.CreateObject("Scripting.FileSystemObject");
		var f;
...
function deleteFile()
{
	...
	file0=Request.queryString("node")(1);
	isDir=Request.queryString("type")(1)=="dir"? true:false;
	file=getRealPath(file0);
	if(delete0(file,isDir)) {
%>
<File>
<Status>true</Status>
</File>
...
<form method="POST" action="?do=modify" name="F" onSubmit="return check()">
	...
	<option value="cookie">Cookie验证</option>
	<option value="HTTP">HTTP Basic验证</option>
	<option value="session">Session会话验证</option>
	<option value="none">无授权验证(不安全！)</option>
	</select> 注意：以CGI方式安装的PHP不支持HTTP Basic验证；IIS下必须关闭“集成Windows身份验证”（在站点属性->目录安全性中设置）才能使用HTTP Basic验证。
	<script>document.F.auth_type.value="<%=CONFIG["auth_type"]%>";</script>
			</td>
		</tr>
		<tr>
			<td width="220" align="right">Cookie有效期　：</td>
			<td height="20"><input type="text" name="cookie_life" size="20" value="<%=CONFIG["cookie_life"]%>"> 以分钟为单位。</td>
		</tr>
		<tr>
			<td width="220" align="right">会话有效期　　：</td>
			<td height="20"><input type="text" name="session_life" size="20" value="<%=CONFIG["session_life"]%>"> 以分钟为单位。</td>
		</tr>
		<tr>
			<td width="220" align="right">目录树加载层数：</td>
			<td height="20"><input type="text" name="nLayer" size="20" value="<%=CONFIG["nLayer"]%>"> 大的层数会加快目录树节点打开速度，但会减慢加载速度。</td>
		</tr>
		<tr>
			<td width="220" align="right">根目录绝对路径：</td>
			<td height="20"><input type="text" name="base_dir" size="20" value="<%=CONFIG["base_dir"]==null?"":CONFIG["base_dir"]%>"> 必须是一个绝对路径，仅当起始目录是以'/'开头时有效。留空表示映射到网站的根目录。</td>
		</tr>
		<tr>
			<td width="220" align="right">起始目录　　　：</td>
			<td height="20"><input type="text" name="start_dir" size="20" value="<%=CONFIG["start_dir"]%>"> 相对于目录树映射的绝对路径的起始目录，可以使用'.'，'/'或指定目录名。</td>
		</tr>
		<tr>
		...
		<tr>
			...
			<td height="20"><input class="box" type="checkbox" name="wrap_word" value="checked" <%=CONFIG["wrap_word"]%>> 如一行超长，使用自动换行将自动折行显示，而不产生滚动条。</td>
		</tr>
		<tr>
			<td width="220" align="right"></td>
			<td>&nbsp;</td>
		...
		<tr>
			<td width="220" align="right">　</td>
			<td><input type="submit" value="提交" name="B1">&nbsp; &nbsp; <input type="reset" value="重置" name="B2">&nbsp; &nbsp; <input type="button" value="返回上一页" onClick="history.back(-1)"></td>
		</tr>
	</table>
</form>
<script>
...
function modifyConfig()
{
	...
	if (Request.form("password")(1)!="")
		AUTH_USER["password"]=MD5(Request.form("password")(1));
	AUTH_USER["email"]=Request.form("email");
	CONFIG["auth_type"]=Request.form("auth_type");
	if (Request.form("base_dir")(1)!="")
		CONFIG["base_dir"]=Request.form("base_dir")(1);
	else
		CONFIG["base_dir"]=null;
	CONFIG["cookie_life"]=Request.form("cookie_life");
	CONFIG["session_life"]=Request.form("session_life");
	CONFIG["nLayer"]=Request.form("nLayer");
	CONFIG["start_dir"]=Request.form("start_dir");
	CONFIG["encoding"]=Request.form("encoding");
	CONFIG["encoding_list"]=Request.form("encoding_list");
	CONFIG["wrap_word"]=Request.form("wrap_word");
	CONFIG["editor_font_family"]=Request.form("editor_font_family");
	CONFIG["editor_font_height"]=Request.form("editor_font_height");
	CONFIG["editor_line_height"]=Request.form("editor_line_height");
	if (Request.form("fckeditor_dir")(1)!="")
		CONFIG["fckeditor_dir"]=Request.form("fckeditor_dir");
	else
		CONFIG["fckeditor_dir"]=null;

	var sStr,src,AbsPath=Server.MapPath(PAGE);
	try {
		src=Server.CreateObject("ADODB.Stream");
		src.Charset="UTF-8";
		src.Type=2;
		src.Mode=3;
		src.Open();
		src.LoadFromFile(AbsPath);
		sStr=src.ReadText();
		src.Close()
		sStr=renewConfig(sStr,"AUTH_USER",serialArray("AUTH_USER"));
		sStr=renewConfig(sStr,"CONFIG",serialArray("CONFIG"));
		src.Open();
		src.WriteText(sStr);
		src.SaveToFile(AbsPath, 2);
	}
	catch (e) {
%><%=HTML_HEADER%>
<body>
无法打开文件 “ <%=AbsPath%> ”。请确认是否有读写此文件的权限。
...
function serialArray(name)
{
	...
	var str="var "+name+"=[];\r\n";
	for (i in arr)
		str+=name+"[\""+i+"\"]="+(arr[i]==null?"null":(typeof(arr[i])=="array"?"[]":"\""+(""+arr[i]).replace(/\\/g, "\\\\")+"\""))+";\r\n";
	return str;
}

function getEncodingCode(encoding)
{
	var i=CONFIG["encoding_list"].indexOf(encoding)+encoding.length+1;
	var j=CONFIG["encoding_list"].indexOf("||",i);
	var code=parseInt(CONFIG["encoding_list"].substring(i,j>=0?j:CONFIG["encoding_list"].length));
	return code;
}

function getNews()
{
	var xmlhttp=new ActiveXObject("MSXML2.ServerXMLHTTP"),R="  ";
	with (xmlhttp) {
		onreadystatechange=function() {
			if (xmlhttp.readyState==4&&xmlhttp.status==200)
				Response.BinaryWrite(xmlhttp.responseBody);
		}
		setTimeouts(4000,4000,4000,8000);
		open("get","http://editone.gaya.cn/ad2RC.php?tmp="+Math.random(),false);
		send();
	}
}
...
{
	if(file.length>0&&file.charAt(0)=='/'&&CONFIG["base_dir"]!=null)
		return CONFIG["base_dir"]+file;
	else
		return Server.MapPath(file);
}
function isExist(file,isDir)
{
	try {
		var fso=Server.CreateObject("Scripting.FileSystemObject");
		if(isDir)
			return fso.FolderExists(file);
		else
...
Function getString(sBin)
	getString=""
	For I=1 to LenB(sBin)
		getString=getString & Chr(AscB(MidB(sBin,I,1)))
	Next
End Function
...
Function uploadFile()
	...
	Dim sBin,src,buf,iPos1,iPos3,iPos4
	sBin=Request.BinaryRead(Request.TotalBytes)
	Set src=Server.CreateObject("ADODB.Stream")
	src.Type=1
	src.Mode=3
	src.Open
	src.Write sBin
	...
	Set buf=Server.CreateObject("ADODB.Stream")
	buf.Type=1
	buf.Mode=3
	buf.Open
	iPos1=InStrB(1, sBin, iCrlf1)-1
	bSeparator=LeftB(sBin, iPos1)
	LenSep=LenB(bSeparator)
	bFile=getByteString("name=""fileUpload""")

	iPos3=1
	DO While true
		iPos3=InStrB(iPos3, sBin, bSeparator)+LenSep
		iPos4=InStrB(iPos3, sBin, iCrLf2)
		bHeader=MidB(sBin, iPos3, iPos4-iPos3)
		If InStrB(1, bHeader, bFile)>0 Then Exit Do
	LOOP

	iPos1=InStrB(iPos3, sBin, iCrlf2)+3
	iPos2=InStrB(iPos3, sBin, bSeparator)-3
	src.Position=iPos1
	src.CopyTo buf, iPos2-iPos1

	filename=Request.QueryString("path")
	filepath=getRealPath(filename)
	buf.SaveToFile filepath,2
	buf.Close
	src.Close
	If Err.Number = 0 Then
		Response.Write "<html><head><meta content='text/html; charset=UTF-8' http-equiv='Content-Type'></head><body><scr"&"ipt>parent.T.uploadReport(true,"""&filename&""",'"&getFileType(filename)&"',"&getTimestamp()&","&(iPos2-iPos1)&");</scr"&"ipt></body></html>"
	Else
		If (fp=fopen(file,"wb")) Then
			fwrite fp,getString(MidB(sBin, iPos1, iPos2-iPos1))
			fclose fp
			Response.Write "<html><head><meta content='text/html; charset=UTF-8' http-equiv='Content-Type'></head><body><scr"&"ipt>parent.T.uploadReport(true,"""&filename&""",'"&getFileType(filename)&"',"&getTimestamp()&","&(iPos2-iPos1)&");</scr"&"ipt></body></html>"
		Else
			Response.Write "<html><head><meta content='text/html; charset=UTF-8' http-equiv='Content-Type'></head><body><scr"&"ipt>parent.T.uploadReport(false,'"&Err.Description&"','"&Err.Number&"');</scr"&"ipt></body></html>"
		End If
	End If
End Function

Function renewConfig(sStr,sName,sContent)
	Dim nB,nE
	nB=InStr(1, sStr, "var "&sName)
	nE=InStr(1, sStr, "//END_"&sName)
	renewConfig=Mid(sStr,1,nB-1)&sContent&Mid(sStr,nE)
End Function
...
Function getEncoding(path)
	...
	twobytes=OBJ.read(2)
	If AscB(MidB(twobytes,1,1))=&HEF And AscB(MidB(twobytes,2,1))=&HBB Then
		getEncoding="UTF-8"
	ElseIf AscB(MidB(twobytes,1,1))=&HFF And AscB(MidB(twobytes,2,1))=&HFE Then
		getEncoding="UNICODE"
	Else
		getEncoding="ANSI"
	End If
...
Function VBDataAdd(min)
	VBDataAdd=DateAdd("n", min, Now)
End Function
</SCRIPT>